...
# Fixtures: zane (Europe/London) works Mon 09:00-18:00 with a 14:30-15:00 break and has
...
class AvailabilityEngineTest < ActiveSupport::TestCase
  ...
  test "monday hours subtract break, appointment and unavailability" do
    assert_equal expected_monday_hours, @engine.available_hours(DATE, @haircut, @zane)
  end

  test "day off (null weekday plan) yields no hours" do
    assert_empty @engine.available_hours("2026-07-22", @haircut, @zane) # Wednesday: null
  end

  test "EA quirk: weekday plan without breaks key yields no periods" do
    plan = JSON.parse(@zane.settings.working_plan)
    plan["monday"] = { "start" => "09:00", "end" => "18:00" } # no breaks key
    @zane.settings.update!(working_plan: plan.to_json)
    assert_empty @engine.available_hours(DATE, @haircut, @zane)
  end

  test "working plan exception overrides weekday hours" do
    WorkingPlanException.create!(provider: @zane, start_date: "2026-07-20", end_date: "2026-07-20",
    ...
    assert_equal %w[10:30 10:45 11:00 11:15 11:30], @engine.available_hours(DATE, @haircut, @zane)
  end

  test "working plan exception day off wins over weekday plan" do
    WorkingPlanException.create!(provider: @zane, start_date: "2026-07-20", end_date: "2026-07-21",
  ...
  test "exception with breaks splits the window" do
    ...
    assert_equal %w[09:00 10:30], @engine.available_hours(DATE, @haircut, @zane)
  end
  ...
  test "single blocked period subtracts hours but does not block the whole date" do
    ...
    hours = @engine.available_hours(DATE, @haircut, @zane)
  ...
  test "multi attendant service allows overlapping bookings up to capacity" do
    2.times do
      Appointment.create!(provider: @zane, customer: users(:jx), service: @group,
                          start_datetime: Time.new(2026, 7, 20, 15, 0, 0),
                          end_datetime: Time.new(2026, 7, 20, 16, 0, 0))
    end
    hours = @engine.available_hours(DATE, @group, @zane)
  ...
  test "invalid date format raises" do
    assert_raises(ArgumentError) { @engine.available_periods("2026/07/20", @zane) }
  end
end
