...
class GoogleCalendarGatewayTest < ActiveSupport::TestCase
  ...
  test "build_event maps EA fields with provider-timezone datetimes" do
    event = @gateway.build_event(appointments(:upcoming), users(:jane), services(:haircut),
                                 users(:james), "Open Out")
    ...
    assert_includes emails, "jane@example.org"
    assert_includes emails, "james@example.org"
  end

  test "build_event falls back to Unavailable and company name" do
    unavailability = appointments(:lunch_block)
    event = @gateway.build_event(unavailability, users(:jane), nil, nil, "Open Out")
