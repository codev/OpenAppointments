...
class BookingCardsTest < ActionDispatch::IntegrationTest
  test "dropdown mode is the default" do
    ...
    assert_select "#select-service:not([hidden])"
  end
  ...
  test "cards mode starts with Select Category, services revealed per category" do
    ...
    assert_select "#category-cards > .booking-card"
    assert_select "#select-service-heading[hidden]", text: I18n.t("ea.select_service")
    assert_select ".service-cards[hidden] .booking-card .booking-card-title", text: services(:haircut).name
    assert_select "#wizard-frame-1 #select-service[hidden]"

    get "/", params: { step: "second", service_id: services(:haircut).id }
    assert_select "#provider-cards"
    assert_select "#wizard-frame-2 #select-provider[hidden]"
  end
  ...
  test "uncategorized services show from the start with the Select Service heading" do
    ...
    get "/"
    assert_select "#select-service-heading:not([hidden])"
    assert_select ".service-cards[data-category-id='']:not([hidden]) .booking-card .booking-card-title",
                  text: "Walk In Trim"
  end
  ...
  test "hidden categories and their services disappear from the booking page" do
    ...
    assert_select ".booking-card .booking-card-title", text: services(:haircut).name, count: 0
  ...
  test "the Select Provider First switch is a button beside Next" do
    ...
    get "/"
    assert_select "#wizard-frame-1 .command-buttons a.swap-first-step",
  ...
  test "cards show the styled picture per the picture style setting" do
    ...
    assert_select ".service-cards .booking-card img.booking-card-picture[src=?]", padded_path
    ...
    assert_select ".service-cards .booking-card img.booking-card-picture[src=?]", zoomed_path
  end
