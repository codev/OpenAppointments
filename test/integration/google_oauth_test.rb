...
class GoogleOauthTest < ActionDispatch::IntegrationTest
  ...
  test "disable_provider_sync clears settings and event ids" do
    jane = users(:jane)
    jane.settings.update!(google_sync: true, google_token: '{"refresh_token":"x"}')
    appointments(:upcoming).update_column(:id_google_calendar, "g1")

    post "/google/disable_provider_sync", params: { provider_id: jane.id }
    assert_equal({ "success" => true }, response.parsed_body)
    assert_not jane.settings.reload.google_sync
    assert_nil jane.settings.google_token
